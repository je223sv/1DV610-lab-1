...
En regel som jag alltid försöker att följa är **Don’t repeat yourself**. Tyvärr, så missade jag att ``generateLetters`` och ``generateSpecialCharacters`` mer eller mindre har samma implementation. I en nästföljande version, så hade jag skapat en generateCharacter funktion som de båda använder för att minimera kodupprepningen.
